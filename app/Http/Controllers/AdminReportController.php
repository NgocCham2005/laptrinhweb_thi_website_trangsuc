...
class AdminReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        $revenueReports = DB::table(
                DB::raw("({$ordersSub->toSql()}) as t")
            )
        ...
        $totalRevenue = DB::table(
            ...
            ->mergeBindings($ordersSub->getQuery())
            ->select(
                DB::raw("SUM(GREATEST(subtotal - COALESCE(GiaTriApDung,0),0)) as total")
            )
            ->value('total') ?? 0;
        ...
        if ($from) {
            $totalRevenueOrdersQuery->whereDate('NgayDatHang', '>=', $from);
        }

        if ($to) {
            $totalRevenueOrdersQuery->whereDate('NgayDatHang', '<=', $to);
        }
        ...
        if ($from) {
            $topProducts->whereDate('don_hang.NgayDatHang', '>=', $from);
        }

        if ($to) {
            $topProducts->whereDate('don_hang.NgayDatHang', '<=', $to);
        }

        if ($product) {
            $topProducts->where('chi_tiet_don_hang.MaSanPham', $product);
        }

        if ($category) {
            $topProducts->where('san_pham.MaDanhMuc', $category);
        }
        ...
        if ($from) {
            $orderReports->whereDate('NgayDatHang', '>=', $from);
        }

        if ($to) {
            $orderReports->whereDate('NgayDatHang', '<=', $to);
        }

        if ($status !== null && $status !== '') {
            $orderReports->where('TrangThai', $status);
        }

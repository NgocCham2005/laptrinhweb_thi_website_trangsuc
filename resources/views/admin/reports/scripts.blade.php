...
new Chart(
    document.getElementById('revenueChart'),
    {
        type: 'bar',
        data: {
            labels: @json($revenueReports->pluck('ngay')),
            datasets: [{
                label: 'Doanh thu',
                data: @json($revenueReports->pluck('doanh_thu')),
                borderWidth: 1
            }]
        }
    }
...
new Chart(
    document.getElementById('orderChart'),
    {
        type: 'line',
        data: {
            labels: @json($orderReports->pluck('ngay')),
            datasets: [{
                label: 'Đơn hàng',
                data: @json($orderReports->pluck('so_don')),
                borderWidth: 2
            }]
        }
    }
...
new Chart(
    document.getElementById('topProductChart'),
    {
        type: 'pie',
        data: {
            labels: @json($topProducts->pluck('TenSanPham')),
            datasets: [{
                data: @json($topProducts->pluck('doanh_thu'))
            }]
        }
    }
);
...
if(categoryFilter && productFilter){
    ...
    categoryFilter.addEventListener(
        ...
        function(){
            ...
            Array.from(productFilter.options)
                .forEach(option => {
